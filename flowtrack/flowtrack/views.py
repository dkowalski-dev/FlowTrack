...
def delete_object(request, model_name, object_id):
    if model_name == "category":
        if (
            Product.objects.filter(owner=request.user, category=object_id).first()
            != None
        ):
            messages.warning(
                request,
                "Nie możesz usunąć kategorii jeżeli masz do niej przypisane produkty",
            )
    ...
    if model_name == "status":
        if Offer.objects.filter(owner=request.user, status=object_id).first() != None:
            messages.warning(
                request,
                "Nie możesz usunąć statusu jeśli masz do niego przypisane oferty",
            )
    ...
    if model_name == "individualclient" or model_name == "companyclient":
        ...
        if (
            Offer.objects.filter(
                owner=request.user, client_id=object_id, client_type=user_type
            ).first()
            != None
        ):
            messages.warning(
                request,
                "Nie możesz usunąć klienta jeśli masz przypisane do niego oferty!",
            )
