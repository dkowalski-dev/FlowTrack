...
def create_product(request):
    if Category.objects.filter(owner=request.user).first() == None:
        messages.warning(
            request, "Aby dodać produkty musisz posiadać przynajmniej jedną kategorię"
        )
...
def categories(request):
    ...
    categories = Category.objects.filter(
        owner=request.user, name__icontains=search_query
    ).order_by(settings.categories_sort)
    categories, page_range = paginObjects(
        request, categories, settings.categories_paginator
    )
...
def update_category(request, pk):
    ...
    return render(
        request, "form_template.html", {"form": form, "title": "Edytuj kategorię"}
    )
