...
def offers(request):
    ...
    offers = list(
        {offer.id: offer for offer in chain(offers_query, offers_clients)}.values()
    )

    if status_filter:
        offers = [offer for offer in offers if offer.status.type in status_filter]
    else:
        offers = [offer for offer in offers if offer.status.type == "ongoing"]
    ...
    if settings.offers_sort == "client":
        offers = sorted(
            offers,
            key=lambda offer: (
                offer.client.company_name.lower()
                if hasattr(offer.client, "company_name")
                else offer.client.name.lower()
            ),
        )
        ...
        offers = sorted(
            offers, key=lambda offer: getattr(offer, sort_key), reverse=reverse
        )
...
def create_offer(request):
    if Status.objects.filter(owner=request.user).first() is None:
        messages.warning(
            request, "Aby utworzyć ofertę musisz dodać przynajmniej jeden status"
        )
...
def offer(request, pk):
    ...
    if request.method == "POST":
        ...
        if "product_form" in request.POST:
            product = OfferProduct.objects.filter(
...
def add_product_to_offer(request, pk):
    ...
    products = products.exclude(
        id__in=offer_products.values_list("product_id", flat=True)
    )
...
def statuses(request):
    ...
    search_query = request.GET.get("search_query", "")
    statuses = (
        Status.objects.filter(owner=request.user)
